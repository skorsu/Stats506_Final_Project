...
independent or not. The logistic regressions were also constructed in this 
analysis by using hypertension as a predictor.

The github pages, along with code used in this analysis, can be found [here](https://github.com/skorsu/Stats506_Final_Project).

## Result

The data in this analysis contained the record of 20,711 people. 40.27% 
of them are those who suffered from hypertension. Table 1 showed the 
...
```{r table2, echo = FALSE}
cap2 <- "Table 2: Result of the logistic regression models, the overall model, 
and all-variable model. For the all-variable model, the percent column 
indicates the change in coefficient when constructing the simple logistic 
regression using only the particular predictor only."
...
```{r table3, echo = FALSE}
cap3 <- "Table 3: Result of the logistic regression models without gender, 
...
will suffer from hypertension if that person had diabetes is 2.04 times higher 
...
that person had diabetes is 1.09 times higher than those who did not have 
diabetes. As a result, diabetes still affects hypertension, but not as high 
as the first thought.

To recapitulate, while diabetes plays a significant role in being a cause 
for the prevalence of hypertension, other aspects can be a cause too. Besides, 
some aspects might affect the prevalence of diabetes, which lead to error in 
identifying the relationship between diabetes and hypertension. These aspects 
need to be excluded from the analysis in order to detect a more precise 
